...
type Props = { movieName: string; numEp: string };

export async function generateMetadata({ params }: { params?: Promise<Props> }, parent: ResolvingMetadata): Promise<Metadata> {
    //get Param
    const { movieName, numEp } = await params || { movieName: '', numEp: '' };
    const regex = /tap(\d+)/;
...
export default async function Watch({ params }: { params?: Promise<Props> }) {
    //get Param
    const { movieName, numEp } = await params || { movieName: '', numEp: '' };
    ...
    const numberEp: string = matchEp[1];
