...
export async function POST(request: NextRequest) {
    try {
        ...
        if (extendedUser?.role === 'admin') {
            ...
            // up episodes
            const episodeList: { result: any[]; filmExisted?: any } = await getYoutubePlaylistItems(
                playlistId,
                extendedUser?.email,
            );
            ...
            if (episodeList.result)
                episodeList.result.forEach(async (episode, index) => {
                    const epUploadRes = await mongodb().db('film').collection('episode').insertOne(episode);
